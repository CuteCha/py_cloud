...
class BiliDownPage(object):
    ...
    def get_pages(self):
        ...
        if response.status_code == 200:
            ...
            if "pages" in content["data"]:
                ...
                for item in pages:
                    infos.append((item["cid"], item["page"], item["part"]))
            else:
                print("no pages in content")
        else:
            print("get_pages error ......")

        return infos

    def get_play_info(self, page, part):
        video_url = None
        audio_url = None
        video_title = None
        try:
            ...
            url = f"https://www.bilibili.com/video/{self.bv}?p={page}"
            response = requests.get(url=url, headers=headers)
            if response.status_code == 200:
                ...
                # 取视频标题
                video_title = title_pattern.search(content).group(1)
                print(f"video_title: {video_title} {page} {part}")
                info_str = info_pattern.search(content).group(1)
                play_info = json.loads(info_str)
                ...
                return video_url, audio_url, f"P{page}_{part}"
        except requests.RequestException:
            print('视频链接错误，请重新更换')
            return video_url, audio_url, f"P{page}_{part}"

    def download_mp4(self, page, part):
        video_url, audio_url, video_title = self.get_play_info(page, part)
        audio_filename = f"{video_title}_a.m4s"
        video_filename = f"{video_title}_v.m4s"
        filename = f"{video_title}.mp4"
        self.download_file(audio_url, audio_filename)
    ...
    def download_mp3(self, page, part):
        video_url, audio_url, video_title = self.get_play_info(page, part)
        audio_filename = f"{video_title}.mp3"
        self.download_file(audio_url, audio_filename)
    ...
    def download_file(self, url, filename):
        try:
            ...
            print("Network problems. Reloading...")
            self.download_file(url, filename)

    def download_list_mp4(self):
        infos = self.get_pages()
        for _, page, part in infos:
            print("=" * 72)
            self.download_mp4(page, part)
            time.sleep(5)

    def download_list_mp3(self):
        infos = self.get_pages()
        for _, page, part in infos:
            print("=" * 72)
            self.download_mp3(page, part)
            time.sleep(5)


def main():
    bv = "BV1Lv411C7aK"
    downloader = BiliDownPage(bv)
    downloader.download_list_mp3()
    # bv = "BV1XM41127ZE"
